...
This file provides guidance to AI coding assistants (GitHub Copilot, Cursor, Codeium, etc.) when working with code in this repository.
...
Keep commit messages short and scoped (e.g., `core/auth: fix token refresh`). Optional hooks in [git_hooks.md](docs/guides/git_hooks.md) can validate strict types, block `Interfaces/`, and warn on `dd()/dump()` or `env()` usage. PRs should list packages touched and call out new routes, config keys, or migrations.
