...
- `listeners` - Event-to-listener mappings
...
test -f routes/api.php && echo "API routes found" || echo "Missing API routes"

# Check for old Interfaces directory
test -d src/Interfaces && echo "Warning: Rename to Contracts/" || echo "Using Contracts/"
```
...
The monorepo includes custom AI tooling in `.claude/`:

- **Custom Skills (Slash Commands):** `/audit`, `/new-crud`, `/add-api`, `/fix-contracts`, `/upgrade-package`, and more
- **Custom Agents:** Specialised agents for package auditing, CRUD scaffolding, API scaffolding, contract generation, code style enforcement
- **Hooks:** Automation scripts for formatting (Laravel Pint), validation, and commit checks
- **MCP Server:** Package intelligence tools (`get_package_info`, `list_packages`, `audit_package`, etc.)
