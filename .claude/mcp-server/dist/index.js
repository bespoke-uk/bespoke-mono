...
async function getPackageInfo(packagePath) {
    ...
    if (category === "blade")
        ...
        type = "template";
    else if (UTILITY_PACKAGES.includes(packageName))
        type = "utility";
    else if (!composer.autoload?.["psr-4"])
        type = "meta";
    // Check for API
...
async function findTraitUsages(traitName) {
    const results = [];
...
async function findModelRelationships(modelName) {
    const relationships = {
        belongsTo: [],
        hasMany: [],
        morphTo: [],
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    switch (name) {
        ...
        case "find_trait_usages": {
            const traitName = args?.trait_name;
        ...
        case "audit_package": {
            ...
            if (info.type === "crud") {
                ...
                if (!info.hasApi)
                    issues.push("Missing API routes");
                else
                    passed.push("Has API routes");
                if (!info.hasExports)
                    issues.push("Missing exports");
                else
                    passed.push("Has exports");
                if (!info.hasImports)
                    issues.push("Missing imports");
                else
                    passed.push("Has imports");
                if (info.contractsCount < 8)
                    issues.push(`Only ${info.contractsCount} contracts (need 8-13+)`);
                else
                    passed.push(`Has ${info.contractsCount} contracts`);
            }
            ...
            if (fs.existsSync(testsPath)) {
                ...
                for (const test of allTests) {
                    if (test.startsWith("Unit/"))
                        coverage.unitTests++;
                    else if (test.startsWith("Feature/"))
                        coverage.featureTests++;
                    else if (test.startsWith("Livewire/"))
                        coverage.livewireTests++;
                }
            }
            const modelsCovered = info.models.filter(m => testFiles.some(t => t.includes(m)));
            return {
                content: [{
                        type: "text",
...
${testFiles.map(f => `- ${f}`).join("\n") || "No tests found"}`,
        ...
        case "compare_packages": {
            const pkg1Name = args?.package1;
            const pkg2Name = args?.package2;
...
| Contracts | ${pkg1.contractsCount} | ${pkg2.contractsCount} |`,
        ...
        case "get_package_health": {
            ...
            // Compliance checks
            const compliance = { passed: [], failed: [] };
            // Basic structure
            ...
            if (info.type === "crud") {
                if (info.hasApi)
                    compliance.passed.push("Has API implementation");
                else
                    compliance.failed.push("Missing API implementation");
                if (info.hasExports)
                    compliance.passed.push("Has exports");
                else
                    compliance.failed.push("Missing exports");
                if (info.hasImports)
                    compliance.passed.push("Has imports");
                else
                    compliance.failed.push("Missing imports");
                if (info.contractsCount >= 8) {
            ...
            const healthScore = Math.round((compliance.passed.length / (compliance.passed.length + compliance.failed.length)) * 100);
